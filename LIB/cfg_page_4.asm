...
EXTMEM_BANK = $0400                   ; 1B No. of current extended memory bank
TEST_VAL    = EXTMEM_BANK + 1         ; 1B
TMP_VAL     = TEST_VAL + 1            ; 1B
TMP_IDX     = TMP_VAL + 1             ; 1B Index for use with buffers, loops
TMP_COUNT   = TMP_IDX + 1             ; 1B For misc temporary counters
TMP_WORD_L  = TMP_COUNT + 1           ; 1B For temporary storage of 16-bit vals
...
BYTE_CONV_L = FUNC_PARAM_H + 1        ; 1B To convert bytes between num & str
BYTE_CONV_H = BYTE_CONV_L + 1         ; 1B

STR_BUF = BYTE_CONV_H + 1             ; 1B

\ MATH STUFF
RAND_SEED    = STR_BUF + STR_BUF_SZ + 1	; 2B random seed number
...
SYS_TIMER_COUNT = STREAM_SELECT_REG + 1 ; 2B times timer interrupt has triggered
SYS_TIMER_INTVL = SYS_TIMER_COUNT + 2   ; 2B value for interval - 2 bytes

...
.hex_str_to_byte              ; assumes text is in BYTE_CONV_H and BYTE_CONV_L
  ...
  cmp #$FF                    ; did we get an error?
  beq hex_str_to_byte_err
  tya                         ; get original result back from Y
...
.hex_str_to_byte_err
  lda #$FF                    ; return #$FF as an error code
  sta FUNC_RESULT
.hex_str_to_byte_end
  ply : pla
  rts
...
.asc_hex_to_bin               ; assumes ASCII char val is in A
  ...
  cmp #6                      ; result should be 0-5
  bcc asc_hex_to_bin_end
.asc_hex_to_bin_err
  lda #$FF                    ; this is an error code
.asc_hex_to_bin_end
